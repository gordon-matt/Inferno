...
public partial class LocalizableStrings
{
    ...
    private async Task SaveAsync(ComparitiveLocalizableString row)
    {
        try
        {
            bool result = await ODataService.PutComparitiveAsync(CultureCode, row);
    ...
    private void ExportLanguagePack() => NavigationManager.NavigateTo($"/admin/localization/localizable-strings/export/{CultureCode}", forceLoad: true);
}